...
gulp.task('shell-all', function () {

	if(typeof assets.tasks.shell == 'undefined' || assets.tasks.shell == null) {
		console.log('No Shell tasks defined. Please add some to assets file');
	} else {

		if (!env.developmentMode) {
			console.log('Not in development mode, "Shell" task disabled.');
		} else {

			assets.tasks.shell.forEach(function(task) {
				// Check if a config is set, use some sensible defaults if not
				return gulp.src('*.js', {read: false})
					.pipe(shell(task.cmd, {}))
					.pipe(gulpif(config.developmentMode, notify({message: 'Shell: ' + task.name})));
			});
